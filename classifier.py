...
part1_train_generator = ImageDataGenerator(rescale=1.0/255).flow_from_directory(train_dir, target_size=(224, 224), batch_size=32, class_mode=None, shuffle=False)
part2_train_generator = ImageDataGenerator(rescale=1.0/255).flow_from_directory(train_dir, target_size=(224, 224), batch_size=32, class_mode='categorical', shuffle=False)
train_categorical_classes = to_categorical(part2_train_generator.classes, num_classes=8)

val_generator = ImageDataGenerator(rescale=1.0/255).flow_from_directory(validation_dir, target_size=(224, 224), batch_size=32, class_mode=None, shuffle=False)
...
model.fit(train_features, train_categorical_classes, epochs=50, batch_size=32, validation_data=(validation_features, validation_categorical_classes), verbose=1)

np.save('categories_dict.npy', val_generator.class_indices)
model.save_weights('weights.h5')

print(model.evaluate(validation_features, validation_categorical_classes, batch_size=32, verbose=1))
